...
const SOURCE_ALLOW = [
  ...
  'Al Jazeera','The Guardian','Axios','MIT Technology Review','VentureBeat','Engadget','IGN','Polygon','Foreign Policy'
];
...
async function resolveFinalUrl(googleNewsUrl) {
  try {
    const r = await fetch(googleNewsUrl, { redirect: 'follow', headers: { 'user-agent': 'Mozilla/5.0' } });
    return r.url || googleNewsUrl;
  } catch { return googleNewsUrl; }
}

async function fetchArticleSnippet(item) {
  try {
    const finalUrl = await resolveFinalUrl(item.link);
...
function distillSummary(title, snippet, source, pubDate) {
  const time = pubDate ? new Date(pubDate).toLocaleString('zh-CN') : 'N/A';
  if (!snippet || snippet.length < 80) {
    return `要点：${title}。来源 ${source}，发布时间 ${time}。`;
  }
  // Essence-style: core claim + why it matters
  const core = snippet
    .replace(/^.*?(?:Summary:|TL;DR:|Lead:)/i,'')
    .replace(/(Subscribe|Sign up|Advertisement).*/ig,'')
    .trim()
    .slice(0, 140);
  return `要点：${core}${core.endsWith('。')?'':'。'}影响：该事件对相关行业与政策预期有直接参考价值。`;
}
...
async function main(){
  ...
  for (const cat of Object.keys(TOPICS)) {
    ...
    for (const p of pick) used.add(p.title.toLowerCase());

    const enriched = [];
    for (const p of pick) {
      const snippet = await fetchArticleSnippet(p);
      enriched.push({
        title: p.title,
        source: p.source,
        pubDate: p.pubDate,
        link: p.link,
        summary: distillSummary(p.title, snippet, p.source, p.pubDate)
      });
    }
    categories[cat] = enriched;
  }

  const report = { date, source:'google-news-rss-live-v3', categories, totalPool: pool.length, totalClean: dedup.length };
  const text = render(report);

  const dir = '/home/moonlight/.openclaw/workspace/daily-briefing/reports';
  await fs.mkdir(dir,{recursive:true});
  await fs.writeFile(`${dir}/${date}-realtime-rss-v3.txt`, text);
  await fs.writeFile(`${dir}/${date}-realtime-rss-v3.json`, JSON.stringify(report,null,2));
  console.log(`${dir}/${date}-realtime-rss-v3.txt`);
}

function render(report){
  let out = `日报（纯实时抓取 v3）- ${report.date}\n`;
