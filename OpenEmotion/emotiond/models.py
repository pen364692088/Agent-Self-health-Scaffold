...
class BodyStateDimensionResponse(BaseModel):
    """MVP-6 D1: Single body state dimension"""
    value: float = 0.5
...
class BodyStateResponse(BaseModel):
    """MVP-6 D1: Virtual body state vector response"""
    energy: BodyStateDimensionResponse = BodyStateDimensionResponse(value=0.7, uncertainty=0.3)
    safety_stress: BodyStateDimensionResponse = BodyStateDimensionResponse(value=0.6, uncertainty=0.4)
    social_need: BodyStateDimensionResponse = BodyStateDimensionResponse(value=0.5, uncertainty=0.5)
    novelty_need: BodyStateDimensionResponse = BodyStateDimensionResponse(value=0.5, uncertainty=0.5)
    focus_fatigue: BodyStateDimensionResponse = BodyStateDimensionResponse(value=0.3, uncertainty=0.4)
    energy_budget_factor: float = 0.84  # Derived from energy for compatibility
...
class PlanResponse(BaseModel):
    ...
    regulation_budget: Optional[float] = None  # MVP-2: cost mechanism state (DEPRECATED: use body_state)
    ...
    body_state: Optional[BodyStateResponse] = None
