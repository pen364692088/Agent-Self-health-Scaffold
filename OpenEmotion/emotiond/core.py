...
# MVP-6 D1: Body State
from emotiond.body_state import BodyStateVector, get_body_state, set_body_state
...
class EmotionState:
    ...
    def __init__(self):
        ...
        # MVP-6 D1: Body State Vector (replaces/supplements energy)
        self.body_state = BodyStateVector()
        # regulation_budget is now derived from body_state.energy
    ...
    def update_from_event(self, event: Event) -> float:
        ...
        # MVP-6 D1: Update body state from event
        subtype = event.meta.get("subtype") if event.meta else None
        self.body_state.update_from_event(event.type, subtype, event.meta)
        # Sync legacy energy with body_state for backward compatibility
        self.energy = self.body_state.energy.value
        ...
        if event.type == "user_message":
            ...
            if subtype == "care":
                ...
                # energy now synced from body_state
    ...
    def apply_homeostasis_drift(self, real_dt: float = 1.0) -> None:
        ...
        # MVP-6 D1: Apply body state time dynamics
        self.body_state.apply_time_passed(real_dt)
        # Sync legacy energy with body_state
        self.energy = self.body_state.energy.value
        # regulation_budget derived from body_state energy for compatibility
        self.regulation_budget = self.body_state.get_energy_budget_factor()
...
async def load_initial_state():
    ...
    # MVP-6 D1: Load body state if available
    body_state_data = db_state.get("body_state")
    if body_state_data:
        emotion_state.body_state = BodyStateVector.from_dict(body_state_data)
        # Sync legacy energy with body_state
        emotion_state.energy = emotion_state.body_state.energy.value
        emotion_state.regulation_budget = emotion_state.body_state.get_energy_budget_factor()
...
async def generate_plan(request: PlanRequest) -> PlanResponse:
    ...
    # MVP-6 D1: Build body state response
    from emotiond.models import BodyStateResponse, BodyStateDimensionResponse
    body_state_response = BodyStateResponse(
        energy=BodyStateDimensionResponse(
            value=emotion_state.body_state.energy.value,
            uncertainty=emotion_state.body_state.energy.uncertainty,
            last_updated=emotion_state.body_state.energy.last_updated
        ),
        safety_stress=BodyStateDimensionResponse(
            value=emotion_state.body_state.safety_stress.value,
            uncertainty=emotion_state.body_state.safety_stress.uncertainty,
            last_updated=emotion_state.body_state.safety_stress.last_updated
        ),
        social_need=BodyStateDimensionResponse(
            value=emotion_state.body_state.social_need.value,
            uncertainty=emotion_state.body_state.social_need.uncertainty,
            last_updated=emotion_state.body_state.social_need.last_updated
        ),
        novelty_need=BodyStateDimensionResponse(
            value=emotion_state.body_state.novelty_need.value,
            uncertainty=emotion_state.body_state.novelty_need.uncertainty,
            last_updated=emotion_state.body_state.novelty_need.last_updated
        ),
        focus_fatigue=BodyStateDimensionResponse(
            value=emotion_state.body_state.focus_fatigue.value,
            uncertainty=emotion_state.body_state.focus_fatigue.uncertainty,
            last_updated=emotion_state.body_state.focus_fatigue.last_updated
        ),
        energy_budget_factor=emotion_state.body_state.get_energy_budget_factor()
    )
    ...
    return PlanResponse(
        ...
        # MVP-6 D1: Body state vector
        body_state=body_state_response
    )
